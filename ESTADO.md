...
## Sesión en progreso 🔧
- Sesión 6, servicios reales — Supabase (login+datos reales) y Resend (correos) YA VERIFICADOS de punta a punta en producción. Hotmart: producto creado, 2 planes ($3.99/mes y $24.99/año), foto de producto puesta, webhook v2.0.0 registrado y HOTTOK guardado en Vercel — pero TODOS los envíos de prueba dan 401 "hottok inválido" en `app/api/webhooks/hotmart/route.ts`. Se agregó extracción del hottok en 4 ubicaciones posibles del payload + un `console.log` de diagnóstico (commit 92b6655) porque el panel de Hotmart no muestra el cuerpo de la respuesta fallida. Confirmado en Vercel Logs que las peticiones SÍ llegan (User Agent "Jodd HTTP" = cliente HTTP de Hotmart), pero aún no se ha visto el contenido del `console.log` (el usuario está navegando el panel de detalle de Vercel Logs buscando la sección "Logs"/consola, no la encontró todavía).
- Próximo paso: leer ese diagnóstico (envConfigured, envLength, recibidoLength, bodyTopLevelKeys, bodyDataKeys) para saber por qué el hottok nunca coincide — sospecha principal: el HOTTOK guardado en Vercel no es exactamente igual al que Hotmart usa (espacio extra, versión vieja regenerada, etc.) o el payload real de Hotmart no trae `hottok` en el body sino solo en un header distinto a `x-hotmart-hottok`.
- Una vez resuelto: quitar el código de debug (comentado como "DEBUG temporal... quitar una vez que funcione" en el route.ts), probar el flujo end-to-end (compra de prueba → suscripción se actualiza en Supabase), y conectar el CTA del paywall a un link real de checkout de Hotmart (hoy el paywall manda a `/login`, no a Hotmart).
