...
Última actualización: 2026-08-02 | Sesión actual: ninguna en curso (Sesión 4 cerrada, esperando OK para Sesión 5)

⏸️ CHECKPOINT — Sesión 4 CERRADA (funnel `/onboarding` → `/paywall` → `/login`, 4 rondas de revisor-visual sobre el paywall, ver detalle abajo en "Sesiones completadas"). Después de cerrarla, el usuario compartió un análisis de patrones de onboarding de otra IA (Gemini) y se evaluó contra la doctrina del SO: se rechazó explícitamente agregar prueba social inventada ("91% reporta alivio", sin usuarias reales — prohibido por `02B-ONBOARDING-Y-PAYWALL.md`) y se descartaron las preguntas de quiz clínico (frecuencia de opresión en el pecho) por rozar terreno de diagnóstico, contra la regla "nunca diagnostica". Sí se implementó el "botón de escape a crisis": nueva ruta `/sos` (respiración guiada 4-4-6, sin login, sin datos falsos) enlazada como "¿Crisis ahora? Toca aquí" desde el header del onboarding, el paywall y el login — ejecuta la regla "nunca" ya vigente de que el SOS es siempre gratis/accesible, que antes solo estaba prometida en el copy pero no como ruta real. tsc+build limpios, probado con Playwright en las 3 pantallas + navegación a /sos, sin errores de consola. / Siguiente acción exacta: proponer Sesión 5 (app interna) — falta confirmación del usuario para arrancarla.
