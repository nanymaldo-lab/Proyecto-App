...
2. El checkout de Hotmart daba "Product unavailable" (error 005) porque el producto seguía en estado "Borrador" — Hotmart exige al menos 1 módulo de contenido publicado en su "Área de Miembros" antes de aprobar el producto para venta, aunque el contenido real viva fuera de Hotmart (nuestra app). Se creó un módulo "Bienvenida" con el link a `/app` y se envió el producto a aprobación. **Hotmart ya aprobó el producto ("Ventas activas") y se confirmó con una prueba real: landing → onboarding → paywall → login → checkout de Hotmart cargando bien (sin error), con los 3 días de prueba y el precio correcto. Flujo de venta end-to-end verificado.**
